 ...
 * Description: Instructor availability handler. Validates and inserts a new availability slot
...
$duplicate_statement->execute([$instructor["user_id"], $availability_date, $start_time]);
$duplicate_slot = $duplicate_statement->fetch(PDO::FETCH_ASSOC);

if ($duplicate_slot) {
    $_SESSION["dashboard_message"] = "That availability slot already exists.";
    $_SESSION["dashboard_message_type"] = "error";
    redirect_to_instructor_dashboard((int) $instructor["user_id"]);
}

$insert_statement = $dbh->prepare(
    "INSERT INTO availability (instructor_id, date, start_time, end_time, is_booked)
     VALUES (?, ?, ?, ?, 0)"
);
