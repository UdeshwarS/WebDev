/*
    ...
    and saves run history in localStorage.
*/
...
window.addEventListener("load", function () {
    ...
    function renderEndScreen() {
        ...
        dom.bestCash.textContent = formatMoney(game.getBestCash());
        renderHistoryList(dom.historyList, game.getRecentHistory(6));
        renderHistoryList(dom.historyModalList, game.history);
    }
...
class TycoonGame {
    ...
    constructor(totalDays) {
        this.totalDays = totalDays;
        this.historyKey = "campus_cart_tycoon_history";
        this.runCountKey = "campus_cart_tycoon_run_count";
     ...
     */
    loadHistory() {
        const historyText = window.localStorage.getItem(this.historyKey);
     ...
     */
    loadRunCount() {
        const savedCount = window.localStorage.getItem(this.runCountKey);
     ...
     */
    saveHistoryEntry() {
        const nextRunNumber = this.loadRunCount() + 1;
        const newEntry = {
            label: "Run " + nextRunNumber,
            cash: this.business.cash,
            cartLevel: this.business.cartLevel,
            popularity: this.business.popularity
        };
        const updatedHistory = [newEntry].concat(this.history);

        window.localStorage.setItem(this.historyKey, JSON.stringify(updatedHistory));
        window.localStorage.setItem(this.runCountKey, String(nextRunNumber));
    }
     ...
     */
    getBestCash() {
        let bestCash = this.business.cash;

        this.history.forEach(function (entry) {
            if (entry.cash > bestCash) {
                bestCash = entry.cash;
            }
        });
    ...
    getRecentHistory(amount) {
        return this.history.slice(0, amount);
    }
}
