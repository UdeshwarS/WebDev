...
/* 
* File: dashboard.php
* Description: Displays the instructor dashboard. Shows today's lessons,
* upcoming lessons, availability slots, cancellation requests, and allows
* instructors to add availability and approve cancellations.
    ...
    <section class="dashboard-grid">
            ...
            <table class="dashboard-table">
                ...
                <tbody>
                    <?php if (count($today_lessons) === 0) { ?>
                        <tr>
                            <td colspan="6" class="empty-table-message">No lessons scheduled for today.</td>
                        </tr>
                    <?php } else { ?>
                        <?php foreach ($today_lessons as $lesson) { ?>
                            <tr class="<?= ((int)$lesson['previous_bookings'] === 0) ? 'first-time-student' : '' ?>">
                                ...
                                <td>
                                    <?php if ($lesson["status"] === "confirmed") { ?>
                                        <form action="mark_complete.php<?= $query_string ?>" method="post"
                                            onsubmit="return confirm('Mark this lesson as completed?');">

                                            <input type="hidden" name="booking_id"
                                                value="<?= htmlspecialchars((string)$lesson["ref_no"]) ?>">
                ...
                <tbody>
                    ...
                    <?php } else { ?>
                        <?php foreach ($upcoming_lessons as $lesson) { ?>
                            <!-- USERS WHO HAVE BOOKED THEIR FIRST LESSON APPEAR A DIFFERENT COLOUR! -->
                            <tr class="<?= ((int)$lesson['previous_bookings'] === 0) ? 'first-time-student' : '' ?>">
                                <td><?= htmlspecialchars($lesson["student_name"]) ?></td>
                                <td><?= htmlspecialchars($lesson["booking_date"]) ?></td>
                                <td><?= htmlspecialchars(substr($lesson["booking_time"], 0, 5)) ?></td>
        ...
        <div class="dashboard-card form-card">
            ...
            <form action="add_availability.php<?= $query_string ?>" method="post" class="dashboard-form">
                ...
                <input type="date" id="availability_date" name="availability_date" min="<?= htmlspecialchars($today_date) ?>" required>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ((int) $slot["is_booked"] === 0) { ?>
                                        <form action="delete_availability.php<?= $query_string ?>" method="post" class="inline-form" onsubmit="return confirm('Delete this availability slot?');">
                                            <input type="hidden" name="availability_id" value="<?= htmlspecialchars((string) $slot["id"]) ?>">
                    ...
                    <?php } else { ?>
                        <?php foreach ($cancellation_requests as $request) { ?>
                            <tr>
                                ...
                                <td>
                                    <form action="approve_cancellation.php<?= $query_string ?>" method="post" class="inline-form" onsubmit="return confirm('Approve this cancellation request?');">
                                        <input type="hidden" name="booking_id" value="<?= htmlspecialchars((string) $request["ref_no"]) ?>">
